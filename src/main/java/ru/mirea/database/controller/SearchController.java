...
public class SearchController {
    ...
    public List<HousingDTO> searchHousing(@RequestParam(name = "num", required = false, defaultValue = "0") int pageNumber,
                                          @RequestParam(name = "size", required = false, defaultValue = "10") int pageSize,
                                          @RequestBody(required = false) HousingSearchDTO housingSearchDTO) {
        Pageable pageable = PageRequest.of(pageNumber, pageSize);
        Page<HousingDTO> page = housingSearchService.search(housingSearchDTO, pageable);
